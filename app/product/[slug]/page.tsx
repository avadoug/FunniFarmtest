      ...
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <FactCard
            icon={FileText}
            label="Batch number"
            value={product.batchNumber || "Pending final package"}
          />
          <FactCard
            icon={FlaskConical}
            label="Lab-tested status"
            value={
              product.labTested
                ? "Lab-tested details available or pending upload."
                : "Testing details pending before live sale."
            }
          />
          <FactCard
            icon={ShieldCheck}
            ...
            value={
              product.ageRestricted
                ...
                : "Age rule pending final compliance review."
            }
          />
        </section>
        ...
        <section className="mt-14 rounded-seed border border-forest-900/12 bg-cream-50 p-6 shadow-farm md:p-8">
          ...
          {includedBundleProducts.length > 0 ? (
            ...
            <p className="mt-6 rounded-2xl border border-dashed border-forest-900/20 bg-white/55 p-5 text-forest-900/70">
              Included products are pending final bundle setup.
            </p>
          )}
        </section>
      )}
      ...
      <div className="mt-14 grid gap-6 lg:grid-cols-[1fr_.82fr]">
        <section className="seed-card rounded-seed p-6 md:p-8">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            <Spec label="Weight" value={product.weight || "Pending"} />
            <Spec label="Pack size" value={product.packSize || "Pending"} />
        ...
        <aside className="space-y-5">
          <Notice icon={ShieldCheck} title="Trust Snapshot" tone="green">
            <ul className="space-y-1">
              ...
              <li>
                {product.labTested
                  ? "Lab-tested and ready for COA review where linked."
          ...
          >
            <p>
              {hasJackFrostCoa
                ...
                : product.coaUrl
                  ? "Use the lab-results link to review available COA notes and placeholders."
                  : "Batch-specific lab files are pending upload."}
            </p>
            {product.coaUrl && (
              <Link
                className="focus-ring mt-3 inline-flex rounded-full font-black text-forest-900 underline decoration-clay/40 underline-offset-4 hover:text-clay"
        ...
        <section className="mt-16">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            ...
            <p className="max-w-2xl text-sm leading-6 text-forest-900/68">
              ...
              like CBG, lab-tested, small batch, or adult use.
            </p>
          </div>
...
function getBestForChips(product: Product) {
  ...
  if (tags.has("lab-tested")) chips.add("Lab transparency shoppers");
...
function getServingInfo(product: Product) {
  ...
  return "Serving details pending final label review.";
}
...
function getProductFaqs(product: Product): Array<[string, string]> {
  return [
    ...
    [
      ...
      product.coaUrl
        ? "Use the COA / Lab Results button on this page to review available lab-result notes, batch details, and transparency placeholders."
        : "Batch-specific COA files are not linked yet. The product should stay in review until matching lab results are added.",
    ],
    [
