...
export const metadata: Metadata = {
  ...
  description:
    "The Funni Farm lab results index with product batch numbers, cannabinoid summaries, lab-tested status, COA links, and plain-English COA reading guidance.",
};
...
export default async function LabResultsPage() {
  ...
  const labProducts = products.filter(
    (product) => product.labTested || Boolean(product.coaUrl),
  );
      ...
      >
        ...
        <div className="p-6 md:p-8">
          <div className="flex flex-wrap gap-2">
            ...
            <Badge tone="gold">COA link pending</Badge>
          ...
          <p className="mt-3 leading-7 text-forest-900/72">
            ...
            batch COA still needs to be connected before live sales.
          </p>
      ...
      <section className="mt-8" aria-labelledby="product-lab-index">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl leading-7 text-forest-900/72">
              Showing active products marked lab-tested and/or products with a
              COA URL in <code>data/products.json</code>.
            </p>
          </div>
          <Link
        ...
        {labProducts.length > 0 ? (
          ...
          <div className="mt-6 rounded-seed border border-dashed border-forest-900/20 bg-cream-50 p-6 text-forest-900/72">
            No active products currently have lab-tested status or COA links.
          </div>
        )}
      </section>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start">
          ...
          <div>
            ...
            <p className="mt-3 leading-7 text-forest-900/72">
              ...
              and current label information. Generic placeholder links should be
              replaced with batch-specific COA files or dedicated lab-result
              anchors before live sales.
            </p>
          </div>
        </div>
      </section>
...
function ProductLabRow({ product }: { product: Product }) {
  ...
  return (
    ...
    >
      ...
      <Td>{product.batchNumber || "Missing"}</Td>
      <Td>{product.cannabinoidInfo || "Pending cannabinoid summary."}</Td>
      <Td>
    ...
    >
      ...
      <div className="mt-4 grid gap-3">
        <MobileField label="Batch Number">
          {product.batchNumber || "Missing"}
        </MobileField>
        <MobileField label="Cannabinoid Info">
          {product.cannabinoidInfo || "Pending cannabinoid summary."}
        </MobileField>
    ...
    <div>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        ...
        {product.labTested ? (
          ...
          <Badge tone="cream">Lab Pending</Badge>
        )}
      </div>
    </div>
  );
...
function LabStatus({ product }: { product: Product }) {
  return (
    ...
    >
      ...
      {product.labTested ? "Lab-tested true" : "Not marked lab-tested"}
    </span>
  );
}
...
function CoaLink({ product }: { product: Product }) {
  if (!product.coaUrl) {
    return <span className="font-bold text-clay">Missing COA URL</span>;
  }

  const isPlaceholder = isPlaceholderCoaUrl(product.coaUrl);
  ...
  return (
    ...
    >
      ...
      {isPlaceholder ? "Lab results index" : "Open COA"}
...
function getLabWarnings(product: Product) {
  if (!product.labTested) return [];

  const warnings: string[] = [];

  if (!product.coaUrl) {
    warnings.push("Marked lab-tested true, but coaUrl is missing.");
  } else if (isPlaceholderCoaUrl(product.coaUrl)) {
    warnings.push(
      "Marked lab-tested true, but coaUrl points to the lab-results index. Connect a batch-specific COA link when available.",
    );
  }

  if (!product.batchNumber) {
    warnings.push("Marked lab-tested true, but batchNumber is missing.");
  } else if (isPlaceholderText(product.batchNumber)) {
    warnings.push(
      "Marked lab-tested true, but batchNumber appears to be a placeholder.",
    );
  }
...
function isPlaceholderCoaUrl(coaUrl: string) {
  return coaUrl.trim() === "/lab-results";
}
