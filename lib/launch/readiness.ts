...
const genericCoaTargets = new Set(["", "/lab-results"]);
...
export const globalLaunchNeeds = [
  {
    ...
    items: [
      "Confirm the live domain so metadata, sitemap, robots.txt, and Open Graph URLs can replace the current example domain.",
  ...
  },
  {
    ...
    items: [
      "Connect the contact form to Resend, SendGrid, Formspree, Supabase, a CRM, or a support inbox.",
...
function addCategoryChecks(
  ...
  if (isCbgProduct && (genericCoaTargets.has(product.coaUrl) || hasUnresolvedText(product.coaUrl))) {
  ...
  if (isCbgProduct && !product.labTested) {
    blockers.add("Product is not marked lab-tested.");
    needed.add("Current third-party lab result before live sales.");
  }
