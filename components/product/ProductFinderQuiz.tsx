...
const questions: Question<keyof Answers>[] = [
  ...
  },
  {
    ...
    options: [
      ...
      {
        description: "A flexible bottle format when label details are final.",
        label: "Oils",
        value: "CBG Oils",
      },
      {
  ...
  },
  {
    ...
    options: [
      ...
      {
        description: "Formats where the final label can guide serving choices.",
      ...
      <section className="seed-card rounded-seed p-5 md:p-6">
        ...
        <div className="mt-6 space-y-5">
          {questions.map((question, index) => (
            <fieldset
              className="rounded-[1.25rem] border border-forest-900/10 bg-white/50 p-4"
              key={question.key}
            >
  ...
  const scored = products
    ...
    .sort((a, b) => {
      ...
      if (Number(b.product.inventory > 0) !== Number(a.product.inventory > 0)) {
        return Number(b.product.inventory > 0) - Number(a.product.inventory > 0);
      }
...
function scoreProduct(product: Product, answers: Answers) {
  ...
  if (product.inventory > 0) score += 2;
  if (product.tags.includes("Lab-Tested")) score += 1;
